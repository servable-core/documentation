...
Servable's design draws directly from DRY, convention-over-configuration, the Liskov substitution principle, and domain-driven design, applied to one specific goal: making a reusable unit of backend capacity (a protocol) as easy to compose and share as a class is to instantiate.
...
See [Servable vs Express, Fastify, NestJS, and plain Parse Server](./comparison) for how this weighs against the alternatives, including when one of them is the better starting point instead. 
