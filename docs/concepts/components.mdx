...
Servable is built from three moving pieces: the server (the orchestrator), protocols (reusable units of backend capacity), and a framework engine (the adapter to the underlying runtime, currently Parse Server). The glossary and manifest below define every term used across this documentation.
...
### Servable server
The Servable server acts as a central orchestrator and mediator. It computes the target schema, checks it's safe to boot (drift + compatibility floor against a committed artifact — see [Schema Migrations](../guides/schema-migrations)) and applies it additively, loads protocols, registers seeding, configuration, routes, jobs, etc.
